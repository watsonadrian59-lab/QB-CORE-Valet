...
RegisterNetEvent('qb-valet:server:GetVehicles', function()
    local src = source
    ...
    1
        ...
        TriggerClientEvent('qb-valet:client:ReceiveVehicles', src, result)
    end)
end)

RegisterNetEvent('qb-valet:server:SpawnVehicle', function(plate)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    MySQL.single('SELECT * FROM player_vehicles WHERE REPLACE(plate, " ", "") = ? AND state = 1', {
        ...
        if Player.Functions.RemoveMoney('bank', Config.ValetFee) then
            TriggerClientEvent('qb-valet:client:SpawnVehicle', src, vehicle)
        else
